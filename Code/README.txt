# Launch the atomapi.py and follow the prompts in the terminal.

# The 'Others' file will have previously used 'final' or in testing products so you're able to see progression in the code over time.

# For API info please see the API_INFO.txt file in this repository

# Please see my Medium posts for reports on the data generated from my python programs :) - https://medium.com/@izik_66261

# My data and programs are used for me and others who read my post to understand and interpret the current status of certain cryptocurrency assets (Within the Cosmos Ecosystem). 

# For extra code on how I get the data for my medium posts, please see the 'Reporting' folder. These are also used for the PowerBI dashboard that simplifies my time creating articles.